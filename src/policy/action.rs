...
use super::qtable::{Action, QTable};
...
pub fn make_qlearning_system(
    ...
    move |obs_r, q, step, last_state, last_action, last_reward, commit, e_act, e_act2| {
        qlearning_update(
            &path,
            is_P1,
            obs_r,
...
pub fn qlearning_update(
    ...
    for obs in obs_r.read() {
        ...
        if let (Some(s_pre), Some(a_pre), Some(r_pre)) =
            (&last_state.s, &last_action.a, &last_reward.r)
        {
            if s != *s_pre {
                // Plan A: update only when move
