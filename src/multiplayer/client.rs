...
pub struct SnapshotBuffer {
    pub snapshots: VecDeque<SnapshotUpdate>, // keep last ~30
}

use crate::{
    app::{IsMainPlayer, MainPlayer},
    player::Player,
};
...
pub fn client_handshake(
    ...
    // snapshots_in -> producer: is the snapshot receiving network task  | consumer: apply_snapshot_system, which receives on server_snapshots.recv at a fixed rate.
    ...
    for packet_number in 0..15 {
        let player_number = if is_main_player.as_ref().0 { 1 } else { 0 };
        let handshake_message =
            HandshakeData::new(packet_number, 0, player_number, "blue_fish", "Sean").encode();
        socket
    ...
    match socket.recv_from(&mut buf) {
        Ok((len, addr)) => {
                    ...
                    loop {
                        match socket_clone.recv_from(&mut buf) {
                            Ok((len, from)) => {
                                ...
                                if let Some(resp) = HandshakeResponse::decode(snapshot) {
                                    if let Some(sent) = send_instants[resp.packet_number as usize] {
                                        ...
                                        println!(
                                            "[Handshake] RTT for packet {} = {} ms",
                                            resp.packet_number, rtt
                                        );
                                    } else {
                                        println!(
                                            "[Handshake] instant not captured for packet {}",
                                            resp.packet_number
                                        );
                                    }
                                } else {
            ...
            commands.insert_resource(ClientNetChannels {
                rx_snapshots: snapshot_receiver,
            });
        }
        Err(e) => {
...
pub fn apply_snapshot_system(
    ...
    is_main_player: Res<IsMainPlayer>,
    mut main_query: Query<(&mut Transform, &mut SmoothTarget), (With<Player>, With<MainPlayer>)>,
    ...
    while let Ok(snapshot) = channels.rx_snapshots.try_recv() {
        ...
        if let (Ok((mut main_transform, mut smooth_main)), Ok((mut other_transform, mut smooth_p2))) = (main_query.single_mut(), other_query.single_mut()) {
            if let Some((x, y)) = snapshot.positions.get(0) {
                if is_main_player.as_ref().0 {
